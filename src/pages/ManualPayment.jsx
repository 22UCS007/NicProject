...
// Define your API base URL here
const API_BASE_URL = 'https://vat-portal-backend-nic.onrender.com/payments'; // Updated API Base URL
...
function ManualPayment() {
  ...
  // State for the current payment entry (Head of Account section)
  const [headOfAccount, setHeadOfAccount] = useState('3-Registration Fee'); // Changed to full string for clarity
  const [paidTowards, setPaidTowards] = useState('VAT Rf');
  const [mode, setMode] = useState('Court Fee');
  const [courtFeeNo, setCourtFeeNo] = useState('');
  const [date, setDate] = useState('');
  ...
  // State for existing payments data (fetched from backend)
  const [payments, setPayments] = useState([]);
  // For temporary payments not yet sent to backend
  const [unsavedPayments, setUnsavedPayments] = useState([]);
  ...
  const fetchPayments = async () => {
    setIsLoading(true);
    setMessage('Fetching previous payments...');
    try {
      const response = await fetch(API_BASE_URL);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      ...
      const data = await response.json();
      // Assuming your API returns an array of payment objects, each with a unique 'id' or 'tin'
      setPayments(data);
      setMessage('Previous payments loaded.');
      console.log('Fetched previous payments:', data); // Log fetched data
    } catch (error) {
      console.error("Error fetching payments from API:", error);
      setMessage(`Failed to fetch previous payments: ${error.message}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAddPayment = () => {
    ...
    const newPaymentData = {
      // Generate a unique client-side ID for temporary tracking before saving to backend
      id: Date.now(),
      tin, // Include TIN from Section 1
      suspenseName, // Include Suspense Name from Section 1
      regType, // Include Reg Type from Section 1
      paymentWith, // Include Payment With from Section 1
      regAcknowledgmentNo, // Include Reg Acknowledgment No from Section 1
      amountToBePaid: parseFloat(amountToBePaid) || 0, // Include Amount To Be Paid from Section 1
      headOfAccount,
      paidTowards,
      mode,
      courtFeeNo,
  ...
  const handleSaveTemporaryPayments = async () => {
    ...
    setIsLoading(true);
    setMessage('Saving payments to backend...');
    try {
      // Prepare data for backend: remove client-side 'id' if backend generates its own
      const dataToSend = unsavedPayments.map(payment => {
        const { id, ...rest } = payment; // Destructure to exclude client-side 'id'
        return rest;
      });

      console.log('Data being sent to backend for saving:', JSON.stringify(dataToSend, null, 2)); // Log data for backend team
      ...
      const response = await fetch(API_BASE_URL, {
        ...
        body: JSON.stringify(dataToSend), // Send the array of payments
      });
      ...
      const result = await response.json(); // Assuming backend returns a success message/object
      console.log('Backend response after saving:', result);
  ...
  // Handler for deleting a payment from the backend
  const handleDeletePayment = async (paymentId) => { // Expecting the unique ID from the backend
    setIsLoading(true);
    setMessage('Deleting payment...');
    try {
      const response = await fetch(`${API_BASE_URL}/${paymentId}`, { // Use ID for DELETE endpoint
        method: 'DELETE',
        headers: {
          // Add any required headers
        },
  ...
  // Handler for deleting a payment from the temporary list (before saving to backend)
  const handleDeleteUnsavedPayment = (id) => {
    setUnsavedPayments(prev => prev.filter(payment => payment.id !== id));
  ...
  return (
    <div className=" bg-red-300  font-inter">
      <div className="w-full mx-auto bg-slate-200  shadow-lg overflow-hidden">
        {/* Header */}
          ...
          <div className="w-2/3 mx-auto p-4 sm:p-6 border-t border-gray-200">
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white border border-black ">
                <thead>
                  <tr className="bg-blue-700 text-left text-sm text-white uppercase tracking-wider">
                    <th className="p-2 border-r border-b border-black">Select</th> {/* Added header */}
                    <th className="p-2 border-r border-b border-black">Delete</th> {/* Added header */}
                    <th className="p-2 border-r border-b border-black">Act</th>
                    <th className="p-2 border-r border-b border-black">Mode</th>
                    <th className="p-2 border-r border-b border-black">Number</th>
                    <th className="p-2 border-r border-b border-black">Date</th>
                ...
                <tbody>
                  {unsavedPayments.map((payment) => (
                    <tr key={payment.id} className="hover:bg-orange-50">
                        <td className="p-2 border-r border-b border-black">
                        <button
                          // No specific action for select in unsaved list, but can be added if needed
                          className="text-blue-600 hover:text-blue-800 font-semibold"
                        >
                          Select
                        </button>
          ...
          <div className="w-2/3 mx-auto p-4 sm:p-6 border-t border-gray-200">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white border border-black ">
                <thead>
                  <tr className="bg-blue-700 text-left text-sm text-white uppercase tracking-wider">
                    <th className="p-2 border-r border-b  border-black">Select</th> {/* Added header for Select */}
                    <th className="p-2 border-r border-b  border-black">Delete</th> {/* Added header for Delete */}
                    <th className="p-2 border-r border-b  border-black">Act</th>
                    <th className="p-2 border-r border-b  border-black">Mode</th>
                    <th className="p-2 border-r border-b  border-black">Number</th>
                    <th className="p-2 border-r border-b  border-black">Date</th>
                ...
                <tbody>
                  {payments.map((payment) => (
                    <tr key={payment.id} className="hover:bg-gray-50"> {/* Assuming 'id' from backend as unique key */}
                      <td className="p-2 border-r border-b  border-black">
                        <button
                          // No direct action here, just for consistency with original design
                          className="text-blue-600 hover:text-blue-800 font-semibold"
                      ...
                      {/* These fields assume your backend payment objects have these properties */}
                      ...
                      <td className="py-2 px-4 border-b border-r border-black">{payment.remarks}</td> {/* Assuming remarks can be bank name */}
